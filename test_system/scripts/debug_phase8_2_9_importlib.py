# test_system/scripts/debug_phase8_2_9_importlib.py
# -*- coding: utf-8 -*-
...
# 使用 importlib 直接导入模块，避免循环导入
pipeline_service = importlib.import_module('app.services.pipeline_service')
result_parser = importlib.import_module('app.services.result_parser')
rules = importlib.import_module('app.rules.rules')
settings = importlib.import_module('configs.settings')

run_workflow = pipeline_service.run_workflow
extract_nested_output = result_parser.extract_nested_output
infer_r1_with_debug = rules.infer_r1_with_debug
EXTRACT_WORKFLOW_ID = settings.EXTRACT_WORKFLOW_ID
S5_WORKFLOW_ID = settings.S5_WORKFLOW_ID
